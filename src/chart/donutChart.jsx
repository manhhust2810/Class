...
import { select, scaleLinear, scaleBand, axisLeft, max, axisBottom, pie, arc, scaleOrdinal } from 'd3'
...
function DonutChart() {
  ...
  function drawBarChar(svgCanvas) {
    ...
    const path = svgCanvas.selectAll('path')
  ...
  function changeSelect(typeData) {
    ...
    if (typeData == 'compute') {
      ...
      const radius = Math.min(width, height) / 2;
      const donutWidth = 75;
      const path = select(svgRef.current)
        .selectAll("path")
        .data(piece); // Compute the new angles
      const arcl = arc()
        .innerRadius(radius - donutWidth)
        .outerRadius(radius);
  ...
  return (
    <div>
      ...
      <div>
        ...
        <button id="clothing" onClick={() => changeSelect('clothing')}>Clothing brand</button>
      </div>
    </div>
  );
}
